...
- 新增独立 SAM2 服务调用文档，完整定义 API 请求/响应、远端路径约束、Python/curl 示例，以及从 GUI/QC 框架拆出后接入其他 pipeline 的节点边界。
- 新增 SAM2.1 Tiny 可移交部署包：固定 PyTorch/CUDA/SAM2/checkpoint 版本，提供 TI-ONE/裸机一键安装、服务启停与健康检查、Dockerfile、配置模板和完整部署说明。
- 新增纯本地人工重锚前后对比视频工具，输出逐 clip 左右对比视频和一份全部案例合并视频。
