...
# path: /pytracking/evalution/tracker.py
...
class Info:
    ...
    init_object_ids: list       #[next_object_id, ],
    object_ids: list           #[next_object_id, ],
    sequence_object_ids: list  # [next_object_id, ]
...
class Tracker:
    ...
    def run_track_first_frame(self, image_data = None, optional_box=None, optional_boxes=None):
        """Run the tracker with the first frame with image_data or a image file.
        args:
            image_data: image data in pybase64 format
        ...
        if not tracker:
            return False, "Init tracker failed", None, None
        ...
        # init multi-boxes as the init_bbox

        if optional_boxes is not None:
            info = {}
            for optional_box in optional_boxes:
                if optional_box is not None:
                    assert isinstance(optional_box, (list, tuple))
                    assert len(optional_box) == 4, "valid box's format is [x,y,w,h]"
                    if not info.get("init_bbox"):
                        info["init_bbox"] = OrderedDict()
                    info["init_bbox"][next_object_id] = optional_box
                    sequence_object_ids.append(next_object_id)
                    output_boxes[next_object_id] = [optional_box, ]
                    next_object_id += 1
            info["init_object_ids"] = sequence_object_ids
            info["object_ids"] = sequence_object_ids
       ...
       # return tracker, prev_output


    def run_track_next_frame(self, image_data = None, image_file_path = None):
        pass
