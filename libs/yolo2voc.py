...
def txtLabel_to_xmlLabel(classes_file, source_pth, save_xml_pth):
    ...
    for file in os.listdir(source_pth):
        if not file.endswith(
                ('.bmp', '.dib', '.png', '.jpg', '.jpeg', '.pbm', '.pgm', '.ppm', '.tif', '.tiff', '.JPG')):
            continue
        img_file = Image.open(os.path.join(source_pth, file))
        txt_file = open(os.path.join(source_pth, file.replace('.JPG', '.txt').replace('.jpg', '.txt').replace('.png',
                                                                                                              '.txt').replace(
            '.PNG', '.txt'))).read().splitlines()
        print(txt_file)
        xml_file = open(os.path.join(save_xml_pth, file.replace('.JPG', '.xml').replace('.jpg', '.xml').replace('.png',
                                                                                                                '.xml').replace(
            '.PNG', '.xml')), 'w')
...
if __name__ == '__main__':
    ...
    classes_file = r"/home/longqi/Downloads/Datasets/data/F22入射50和60/F22_入射50_大/classes.txt"
    #    txtLabel_to_xmlLabel(r'/home/saiki/Documents/test-help/',r'/home/saiki/Documents/test-help/xml/')
    txtLabel_to_xmlLabel(classes_file, r'/home/longqi/Downloads/Datasets/data/F22入射50和60/F22_入射60_small',
                         r'/home/longqi/Downloads/Datasets/data/F22入射50和60/F22_入射60_small/xml/')
